...
#ifdef SERIALIZE
integer:: mode, abskord, iep1, iedp1, jedp1, js2d, o3mr, sgs_tke
real :: qmin
qmin = 0.0
iep1=ie+1
iedp1=ied+1
...
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    ...
    call fs_write_field(ppser_serializer, ppser_savepoint, 'qliquid_js', q(:,j,:,liq_wat))
    call fs_write_field(ppser_serializer, ppser_savepoint, 'qice_js', q(:,j,:,ice_wat))
    call fs_write_field(ppser_serializer, ppser_savepoint, 'qrain_js', q(:,j,:,rainwat))
    call fs_write_field(ppser_serializer, ppser_savepoint, 'qsnow_js', q(:,j,:,snowwat))
    call fs_write_field(ppser_serializer, ppser_savepoint, 'qgraupel_js', q(:,j,:,graupel))
    call fs_write_field(ppser_serializer, ppser_savepoint, 'qcld_js', q(:,j,:,cld_amt))
    call fs_write_field(ppser_serializer, ppser_savepoint, 'gz1d', gz)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'cvm', cvm)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'r_vir', r_vir)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'cappa', cappa)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'rrg', rrg)
  ...
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qvapor_js', q(:,j,:,sphum))
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qliquid_js', q(:,j,:,liq_wat))
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qice_js', q(:,j,:,ice_wat))
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qrain_js', q(:,j,:,rainwat))
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qsnow_js', q(:,j,:,snowwat))
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qgraupel_js', q(:,j,:,graupel))
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qcld_js', q(:,j,:,cld_amt))
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'gz1d', gz)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'cvm', cvm)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'r_vir', r_vir)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'cappa', cappa)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'rrg', rrg)
    ...
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'j_2d', js2d)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qvapor_js', q(:,j,:,sphum), ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qliquid_js', q(:,j,:,liq_wat), ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qice_js', q(:,j,:,ice_wat), ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qrain_js', q(:,j,:,rainwat), ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qsnow_js', q(:,j,:,snowwat), ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qgraupel_js', q(:,j,:,graupel), ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qcld_js', q(:,j,:,cld_amt), ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'gz1d', gz, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'cvm', cvm, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'r_vir', r_vir, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'cappa', cappa, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'rrg', rrg, ppser_zrperturb)
    ...
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'j_2d', js2d, ppser_zrperturb)
END SELECT
endif
#endif
               do k=1,km
...
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    ...
    call fs_write_field(ppser_serializer, ppser_savepoint, 'pt', pt)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'gz1d', gz)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'pn2', pn2)
  ...
  CASE(1)
    ...
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pt', pt)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'gz1d', gz)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pn2', pn2)
  ...
  CASE(2)
    ...
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pt', pt, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'gz1d', gz, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pn2', pn2, ppser_zrperturb)
...
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    ...
    call fs_write_field(ppser_serializer, ppser_savepoint, 'pe1', pe1)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'pe2', pe2)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'dp2', dp2)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'q_min', qmin)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'j_2d', js2d)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'nq', nq)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qtracers', q(:,:,:,1:nq))
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pe1', pe1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pe2', pe2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'dp2', dp2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'q_min', qmin)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'j_2d', js2d, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'nq', nq, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qtracers', q(:,:,:,1:nq), ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pe1', pe1, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pe2', pe2, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'dp2', dp2, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'q_min', qmin, ppser_zrperturb)
END SELECT
endif
#endif
...
#ifdef SERIALIZE
   real:: peln(is:ie,km+1,js:je)  !< log(pe)
#else
   real, intent(in):: peln(is:ie,km+1,js:je)  !< log(pe)
#endif
#ifdef SERIALIZE
   real:: cp, rg, r_vir, hlv
#else
...
#ifdef SERIALIZE
      real ::  pe1(i1:i2,km+1)     !< height at layer edges from model top to bottom surface
#else
      real, intent(in) ::  pe1(i1:i2,km+1)     !< height at layer edges from model top to bottom surface
#endif
#ifdef SERIALIZE
      real ::  pe2(i1:i2,kn+1)     !< height at layer edges from model top to bottom surface
#else
...
#endif
#ifdef SERIALIZE
 real ::  pe2(i1:i2,kn+1)  !< pressure at layer edges from model top to bottom surface in the new vertical coordinate
#else
...
#endif
#ifdef SERIALIZE
      real::  pe2(i1:i2,km+1)     !< pressure at layer edges from model top to bottom surface in the new vertical coordinate
#else
...
#ifdef SERIALIZE
 real   :: delp(i1:i2,km)     !< Layer pressure thickness
#else
...
! !INPUT PARAMETERS:
...
#ifdef SERIALIZE
      real ::  pe1(i1:i2,km+1)  ! pressure at layer edges
#else
      real, intent(in) ::  pe1(i1:i2,km+1)  ! pressure at layer edges
                                               ...
                                               ! in the original vertical coordinate
#ifdef SERIALIZE
      real ::  pe2(i1:i2,kn+1)  ! pressure at layer edges
#else
