...
#ifdef SERIALIZE
   real, OPTIONAL:: mass(bd%isd:bd%ied,bd%jsd:bd%jed)
#else
...
! file: /home/mad/work/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp/@fvdycore/model/tp_core.F90.SER lineno: #267
    ...
    call fs_write_kbuff(ppser_serializer, ppser_savepoint, 'mass', mass, k=k, k_size=nz, mode=ppser_get_mode())
...
#ifdef SERIALIZE
      real, optional:: q(ifirst:ilast,jfirst:jlast,kfirst:klast,nq)
#else
...
!------------------
   ...
   type(fv_grid_type), intent(IN), target :: gridstruct
#ifdef SERIALIZE
   real, optional:: mass(bd%isd:bd%ied, bd%jsd:bd%jed)  ! q ghosted on input
#else
