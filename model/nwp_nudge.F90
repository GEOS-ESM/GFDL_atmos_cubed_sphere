module nwp_nudge_mod
 ...
 public do_nwp_nudge, nwp_nudge_init, nwp_nudge_end, breed_slp_inline
...
! track dataset: 'INPUT/tropical_cyclones.txt'
  ...
  real*4::     x_obs(nobs_max,max_storm)           ! longitude in degrees
  real*4::     y_obs(nobs_max,max_storm)           ! latitude in degrees
  real*4::  mslp_obs(nobs_max,max_storm)           ! observed SLP in mb
  real*4::  mslp_out(nobs_max,max_storm)           ! outer ring SLP in mb
  real*4::   rad_out(nobs_max,max_storm)           ! outer ring radius in meters
  real*4::   time_tc(nobs_max,max_storm)           ! start time of the track
!------------------------------------------------------------------------------------------

 namelist /nwp_nudge_nml/ nudge_virt, nudge_hght, nudge_t, nudge_q, nudge_winds, nudge_tpw, &
                          tau_winds, tau_t, tau_q, tau_virt, tau_hght,  kstart, kbot_winds, &
                          k_breed, k_trop, p_trop, kord_data, tc_mask, nudge_debug, nf_t,    &
                          nf_uv, breed_vortex, tau_vortex, tau_tpw, strong_mask,   &
...
!----------------------------------------
      call get_slp_obs(time, nobs_tc(n), x_obs(1,n), y_obs(1,n), mslp_obs(1,n), mslp_out(i,n), rad_out(1,n),   &
                       time_tc(1,n), pos(1), pos(2), slp_o, r_vor, p_vor)

      if ( slp_o<880.E2 .or. slp_o>min(slp_env,slp_mask) .or. abs(pos(2))*rad2deg>38. ) goto 5000  ! next storm
...
!     if ( p_env < max(slp_o, 1000.E2) ) then
      if ( p_env < max(1000.E2, slp_o+250.0) ) then
...
!                             pdep = max(1.0, min(abs(0.5*delps), 0.020*delp(i,j,k)))
                              pdep = max(1.0, min(abs(0.4*delps), 0.025*delp(i,j,k)))
