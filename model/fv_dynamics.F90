...
#ifdef SERIALIZE
    integer :: n_split        !< small-step horizontal dynamics
#else
    integer, intent(IN) :: n_split        !< small-step horizontal dynamics
#endif
#ifdef SERIALIZE
    integer :: q_split        !< tracer
#else
...
#ifdef SERIALIZE
    logical :: hybrid_z       !< Using hybrid_z for remapping
#else
...
#ifdef SERIALIZE
integer:: mode, n_map_step
real :: ph1v(npz), ph2v(npz)
#endif
...
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    ...
    call fs_write_field(ppser_serializer, ppser_savepoint, 'n_split', n_split)
    ...
    call fs_write_field(ppser_serializer, ppser_savepoint, 'phis', phis)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'wsd', ws)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'omga', omga)
  ...
  CASE(1)
    ...
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'n_split', n_split)
    ...
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'phis', phis)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'wsd', ws)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'omga', omga)
    ...
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ua', ua)
  ...
  CASE(2)
    ...
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'n_split', n_split, ppser_zrperturb)
    ...
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'phis', phis, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'wsd', ws, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'omga', omga, ppser_zrperturb)
    ...
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ua', ua, ppser_zrperturb)
...
SELECT CASE ( ppser_get_mode() )
  ...
  CASE(2)
    ...
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ak', ak, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'bk', bk, ppser_zrperturb)
    ...
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'n_map', n_map_step, ppser_zrperturb)
END SELECT
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'dpx', dpx)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'dpx', dpx)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'dpx', dpx, ppser_zrperturb)
END SELECT
#endif
...
#ifdef SERIALIZE
call fs_create_savepoint('DryMassControl-Out', ppser_savepoint)
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'pe', pe)
  CASE(1)
...
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'mfxd_R4', mfxL)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'mfyd_R4', mfyL)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'cxd_R4', cxL)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'cyd_R4', cyL)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'mfxd_R4', mfxL)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'mfyd_R4', mfyL)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'cxd_R4', cxL)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'cyd_R4', cyL)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'mfxd_R4', mfxL, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'mfyd_R4', mfyL, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'cxd_R4', cxL, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'cyd_R4', cyL, ppser_zrperturb)
END SELECT
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'nq', nq)
...
#ifdef SERIALIZE
call fs_create_savepoint('Remapping_GEOS-In', ppser_savepoint)
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'iq', iq)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'last_step', last_step)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'consv_te', consv_te)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'ps', ps)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'pe', pe)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'delp', delp)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'pkz', pkz)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'pk', pk)
  ...
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'iq', iq)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'last_step', last_step)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'consv_te', consv_te)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ps', ps)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pe', pe)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'delp', delp)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pkz', pkz)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pk', pk)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'iq', iq, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'last_step', last_step, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'consv_te', consv_te, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ps', ps, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pe', pe, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'delp', delp, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pkz', pkz, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pk', pk, ppser_zrperturb)
...
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'sphum', sphum)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'q_con', q_con)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'u', u)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'v', v)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'w', w)
    ...
    call fs_write_field(ppser_serializer, ppser_savepoint, 'akap', akap)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'sphum', sphum)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'q_con', q_con)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'u', u)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'v', v)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'w', w)
    ...
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'akap', akap)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'sphum', sphum, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'q_con', q_con, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'u', u, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'v', v, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'w', w, ppser_zrperturb)
...
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'cappa', cappa)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'kord_tracer', kord_tracer)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'peln', peln)
  ...
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'cappa', cappa)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'kord_tracer', kord_tracer)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'peln', peln)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'te_2d', te_2d)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ua', ua)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'va', va)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'omga', omga)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'dp1', dp1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'cappa', cappa, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'kord_tracer', kord_tracer, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'peln', peln, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'te_2d', te_2d, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ua', ua, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'va', va, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'omga', omga, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'dp1', dp1, ppser_zrperturb)
...
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'reproduce_sum', reproduce_sum)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'ptop', ptop)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'ak', ak)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'bk', bk)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'pfull', pfull)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'hybrid_z', hybrid_z)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'reproduce_sum', reproduce_sum)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ptop', ptop)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ak', ak)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'bk', bk)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pfull', pfull)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'hybrid_z', hybrid_z)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'reproduce_sum', reproduce_sum, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ptop', ptop, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ak', ak, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'bk', bk, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pfull', pfull, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'hybrid_z', hybrid_z, ppser_zrperturb)
END SELECT
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'do_adiabatic_init', do_adiabatic_init)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'nq', nq)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'do_adiabatic_init', do_adiabatic_init)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'nq', nq)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'do_adiabatic_init', do_adiabatic_init, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'nq', nq, ppser_zrperturb)
END SELECT
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'mfx_R4', mfxL)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'mfy_R4', mfyL)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'cx_R4', cxL)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'cy_R4', cyL)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'mfx_R4', mfxL)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'mfy_R4', mfyL)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'cx_R4', cxL)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'cy_R4', cyL)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'mfx_R4', mfxL, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'mfy_R4', mfyL, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'cx_R4', cxL, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'cy_R4', cyL, ppser_zrperturb)
END SELECT
#endif
...
#ifdef SERIALIZE
call fs_create_savepoint('Remapping_GEOS-Out', ppser_savepoint)
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'te_2d', te_2d)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'pk', pk)
...
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    ...
    call fs_write_field(ppser_serializer, ppser_savepoint, 'qliquid', q(:,:,:,liq_wat))
    call fs_write_field(ppser_serializer, ppser_savepoint, 'qice', q(:,:,:,ice_wat))
    call fs_write_field(ppser_serializer, ppser_savepoint, 'qrain', q(:,:,:,rainwat))
    call fs_write_field(ppser_serializer, ppser_savepoint, 'qsnow', q(:,:,:,snowwat))
    call fs_write_field(ppser_serializer, ppser_savepoint, 'qgraupel', q(:,:,:,graupel))
    call fs_write_field(ppser_serializer, ppser_savepoint, 'qcld', q(:,:,:,cld_amt))
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'peln', peln)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'delz', delz)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pt', pt)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'delp', delp)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qvapor', q(:,:,:,sphum))
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qliquid', q(:,:,:,liq_wat))
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qice', q(:,:,:,ice_wat))
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qrain', q(:,:,:,rainwat))
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qsnow', q(:,:,:,snowwat))
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qgraupel', q(:,:,:,graupel))
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qcld', q(:,:,:,cld_amt))
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'peln', peln, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'delz', delz, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pt', pt, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'delp', delp, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qvapor', q(:,:,:,sphum), ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qliquid', q(:,:,:,liq_wat), ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qice', q(:,:,:,ice_wat), ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qrain', q(:,:,:,rainwat), ppser_zrperturb)
...
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    ...
    call fs_write_field(ppser_serializer, ppser_savepoint, 'ua', ua)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'va', va)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'mode', mode)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'u', u)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'v', v)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ua', ua)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'va', va)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'mode', mode)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'u', u, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'v', v, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ua', ua, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'va', va, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'mode', mode, ppser_zrperturb)
END SELECT
#endif
...
#ifdef SERIALIZE
    real :: phis(bd%isd:bd%ied,bd%jsd:bd%jed)     !< Surface geopotential (g*Z_surf)
#else
