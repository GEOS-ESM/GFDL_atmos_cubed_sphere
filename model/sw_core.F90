...
#ifdef SERIALIZE
use k_checkpoint, only: get_k,get_nz
#endif
...
SELECT CASE ( ppser_get_mode() )
  ...
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'dord4', dord4)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'dord4', dord4, ppser_zrperturb)
END SELECT
endif
#endif
...
call fs_create_savepoint('VorticityTransport_Cgrid-In', ppser_savepoint)
    ...
    call fs_write_kbuff(ppser_serializer, ppser_savepoint, 'vc', vc, k=k, k_size=nz, mode=ppser_get_mode())
    call fs_write_kbuff(ppser_serializer, ppser_savepoint, 'vort_c', vort, k=k, k_size=nz, mode=ppser_get_mode())
    call fs_write_kbuff(ppser_serializer, ppser_savepoint, 'ke_c', ke, k=k, k_size=nz, mode=ppser_get_mode())
    call fs_write_kbuff(ppser_serializer, ppser_savepoint, 'v', v, k=k, k_size=nz, mode=ppser_get_mode())
    call fs_write_kbuff(ppser_serializer, ppser_savepoint, 'u', u, k=k, k_size=nz, mode=ppser_get_mode())
...
#ifdef SERIALIZE
      integer:: hord_tr, hord_mt, hord_vt, hord_tm, hord_dp
#else
...
#ifdef SERIALIZE
      integer:: sphum, nq, k, km
#else
      integer, intent(IN):: sphum, nq, k, km
#endif
#ifdef SERIALIZE
      real   :: dt, dddmp, d2_bg, d4_bg, d_con
#else
...
!------------------------
      real(kind=8), intent(INOUT)::    cx(bd%is:bd%ie+1,bd%jsd:bd%jed  )
      real(kind=8), intent(INOUT)::    cy(bd%isd:bd%ied,bd%js:bd%je+1)
      logical, intent(IN):: hydrostatic
...
!---
      ...
      logical :: nested
#ifdef SERIALIZE
integer :: mpi_rank,ier
integer :: nz, dir
real, dimension(1,1) :: damp_v_dup, nord_v_dup, nord_dup, nord_w_dup, damp4_dup, d2_bg_dup
damp_v_dup(1,1)=damp_v
nord_v_dup(1,1)=nord_v
nord_dup(1,1)=nord
nord_w_dup(1,1)=nord_w
d2_bg_dup(1,1)=d2_bg
...
call fs_create_savepoint('FxAdv-In', ppser_savepoint)
    ...
    call fs_write_kbuff(ppser_serializer, ppser_savepoint, 'xfx_adv', xfx_adv, k=k, k_size=nz, mode=ppser_get_mode())
...
call fs_create_savepoint('FvTp2d-2-In', ppser_savepoint)
    ...
    call fs_write_kbuff(ppser_serializer, ppser_savepoint, 'xfx', xfx_adv, k=k, k_size=nz, mode=ppser_get_mode())
...
SELECT CASE ( ppser_get_mode() )
  ...
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'hord', hord_dp)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'hord', hord_dp, ppser_zrperturb)
END SELECT
endif
#endif
...
call fs_create_savepoint('Del6VtFlux-In', ppser_savepoint)
    ...
    call fs_write_kbuff(ppser_serializer, ppser_savepoint, 'wd2', wk, k=k, k_size=nz, mode=ppser_get_mode())
    call fs_write_kbuff(ppser_serializer, ppser_savepoint, 'nord_w', nord_w_dup, k=k, k_size=nz, mode=ppser_get_mode())
    call fs_write_kbuff(ppser_serializer, ppser_savepoint, 'damp4', damp4_dup, k=k, k_size=nz, mode=ppser_get_mode())
#endif
...
SELECT CASE ( ppser_get_mode() )
  ...
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'dd8', dd8)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'dd8', dd8, ppser_zrperturb)
END SELECT
endif
#endif
                 do j=js,je
...
call fs_create_savepoint('Wdivergence-In', ppser_savepoint)
    ...
    call fs_write_kbuff(ppser_serializer, ppser_savepoint, 'gy', gy, k=k, k_size=nz, mode=ppser_get_mode())
#endif
            do j=js,je
               do i=is,ie
...
SELECT CASE ( ppser_get_mode() )
  ...
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'hord', hord_tm)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'hord', hord_tm, ppser_zrperturb)
END SELECT
endif
#endif
...
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    ...
    call fs_write_field(ppser_serializer, ppser_savepoint, 'dt4', dt4)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'dt5', dt5)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'dt4', dt4)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'dt5', dt5, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'dt4', dt4, ppser_zrperturb)
END SELECT
endif
#endif
...
call fs_create_savepoint('DivergenceDamping-In', ppser_savepoint)
    ...
    call fs_write_kbuff(ppser_serializer, ppser_savepoint, 'z_rat', z_rat, k=k, k_size=nz, mode=ppser_get_mode())
...
call fs_create_savepoint('Del6VtFlux-In', ppser_savepoint)
    ...
    call fs_write_kbuff(ppser_serializer, ppser_savepoint, 'wd2', vort, k=k, k_size=nz, mode=ppser_get_mode())
    call fs_write_kbuff(ppser_serializer, ppser_savepoint, 'nord_w', nord_v_dup, k=k, k_size=nz, mode=ppser_get_mode())
    call fs_write_kbuff(ppser_serializer, ppser_savepoint, 'damp4', damp4_dup, k=k, k_size=nz, mode=ppser_get_mode())
#endif
...
#endif
#ifdef SERIALIZE
  real ::  u(bd%isd:bd%ied,bd%jsd:bd%jed+1)
#else
  real, intent(in) ::  u(bd%isd:bd%ied,bd%jsd:bd%jed+1)
#endif
#ifdef SERIALIZE
  real ::  v(bd%isd:bd%ied+1,bd%jsd:bd%jed)
#else
